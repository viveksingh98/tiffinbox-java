...
public final class ContextReport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (AnnotationConfigApplicationContext ctx =
            ...
            header(stable, "c4-unit09/exercise");
            List<String> mine = appBeans(ctx.getBeanFactory());
            printBeans(ctx, mine, stable);

            System.out.println("claims");
            // The row this unit stands on: a prototype is never in the singleton store, which
            // is why every row for it above carries the (not built) marker.
            var bf3 = ctx.getBeanFactory();
            var runDef = bf3.getBeanDefinition("deliveryRun");
            claim("deliveryRun scope in the definition",
                    "\"" + runDef.getScope() + "\"  isSingleton=" + runDef.isSingleton());
            claim("container holds a singleton for it", String.valueOf(
                    bf3.containsSingleton("deliveryRun")));
            ...
            claim("trip one == trip two", String.valueOf(t1 == t2));
            claim("stops on the object trip two returned", String.valueOf(t2.stops()));
            claim("DeliveryRun objects this JVM built", String.valueOf(DeliveryRun.built()));
            claim("prototype definitions in this context",
                    String.valueOf(prototypeCount(ctx, mine)));
        }
    }

    /** DERIVED over the SAME filtered list the report printed. */
    static int prototypeCount(AnnotationConfigApplicationContext ctx, List<String> mine) {
        int n = 0;
        for (String name : mine) {
            if (ctx.getBeanFactory().getBeanDefinition(name).isPrototype()) {
                n++;
            }
        }
        if (n == 0) {
            die("0 prototype definitions in the unit whose subject is prototype scope. Either "
                    + "the scope annotation is gone or this is not the configuration class the "
