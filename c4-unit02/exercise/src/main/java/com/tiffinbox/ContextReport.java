...
public final class ContextReport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (AnnotationConfigApplicationContext ctx =
            ...
            header(stable, "c4-unit02/exercise");
            List<String> mine = appBeans(ctx.getBeanFactory());
            printBeans(ctx, mine, stable);

            System.out.println("claims");
            // DERIVED by this run, not typed: the same reflection the previous course wrote.
            int declared = com.tiffinbox.handlers.Handlers.class.getDeclaredMethods().length;
            int labelled = 0;
            for (java.lang.reflect.Method m : com.tiffinbox.handlers.Handlers.class.getDeclaredMethods()) {
                if (m.getAnnotation(com.tiffinbox.handlers.Route.class) != null) {
                    labelled++;
                }
            }
            if (declared == 0) {
                die("getDeclaredMethods() returned nothing for Handlers, so the comparison below "
                        + "has no left-hand side.");
            }
            claim("routes the hand-written scanner finds", labelled + " of " + declared + " methods");
            claim("beans of type Dashboard", String.valueOf(
                    ctx.getBeanNamesForType(Dashboard.class).length));
            claim("dashboard.load().customers()", String.valueOf(
                    ctx.getBean(Dashboard.class).load().customers().size()));
            // DERIVED: count the @Bean methods on the description rather than asserting four.
            int beanMethods = 0;
            for (java.lang.reflect.Method m : TiffinBoxConfig.class.getDeclaredMethods()) {
                if (m.getAnnotation(org.springframework.context.annotation.Bean.class) != null) {
                    beanMethods++;
                }
            }
            claim("@Bean methods in the description", String.valueOf(beanMethods));
            // The two claims THIS unit adds, and the diff is the unit's proof.
            claim("getBean(name) == getBean(type)", String.valueOf(
                    ctx.getBean("customerRepository") == ctx.getBean(CustomerRepository.class)));
            // COUNTED, not computed: a subtraction is only right while BOTH terms are.
            claim("definitions with a null beanClassName",
                    String.valueOf(countWithoutClassName(ctx, mine)));
        }
    }
