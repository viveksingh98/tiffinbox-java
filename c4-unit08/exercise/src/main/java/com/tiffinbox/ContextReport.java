...
public final class ContextReport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (AnnotationConfigApplicationContext ctx =
                     new AnnotationConfigApplicationContext(FreezerConfig.class)) {
            header(stable, "c4-unit08/exercise");
            List<String> mine = appBeans(ctx.getBeanFactory());
            printBeans(ctx, mine, stable);

            System.out.println("claims");
            var fd = ctx.getBeanFactory().getBeanDefinition("freezer");
            claim("freezer scope in the definition",
                    "\"" + fd.getScope() + "\"  isSingleton=" + fd.isSingleton());
            claim("destroy method recorded in the definition",
                    String.valueOf(fd.getDestroyMethodName()));
            claim("destroy hooks the freezer declares", String.valueOf(declaredHooks()));
            claim("destroy hooks the container WILL run on it",
                    String.valueOf(fd.isSingleton() ? declaredHooks() : 0));
        }
    }

    /** DERIVED off the class: a @PreDestroy method plus an inferable close(). */
    static int declaredHooks() {
        int n = 0;
        for (java.lang.reflect.Method m : Freezer.class.getDeclaredMethods()) {
            if (m.getAnnotation(jakarta.annotation.PreDestroy.class) != null) {
                n++;
            }
        }
        if (AutoCloseable.class.isAssignableFrom(Freezer.class)) {
            n++;
        }
        return n;
    }
