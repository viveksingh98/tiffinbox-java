...
cap name-right  java -cp "$CP" com.tiffinbox.TheName right
cap name-wrong  java -cp "$CP" com.tiffinbox.TheName wrong
cap ph-default  java -cp "$CP" com.tiffinbox.ThePlaceholder withDefault
cap ph-literal  java -cp "$CP" com.tiffinbox.ThePlaceholder noConfigurer
cap ph-named    java -cp "$CP" com.tiffinbox.ThePlaceholder configured

# DERIVED, not asserted: does the misnamed-bean failure ever mention the bean it is about?
m=$(grep -c "onversionService" .r-name-wrong.out || true)
s=$(grep -c "^config=\|^conversion service beans" .r-name-wrong.out || true)
echo
echo "  \"onversionService\" in the whole name-wrong capture: $m"
echo "  ...printed by this program itself:                   $s"
echo "  => mentions inside the exception chain:              $((m - s))"
