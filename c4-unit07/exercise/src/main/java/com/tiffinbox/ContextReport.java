...
public final class ContextReport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (AnnotationConfigApplicationContext ctx =
            ...
            header(stable, "c4-unit07/exercise");
            List<String> mine = appBeans(ctx.getBeanFactory());
            printBeans(ctx, mine, stable);

            System.out.println("claims");
            Kitchen k = ctx.getBean(Kitchen.class);
            claim("the kitchen opened", "yes");
            claim("what it knew its name to be at that moment", Kitchen.nameWhenOpened);
            claim("it knew its own name when it opened", String.valueOf(k.knewItsName()));
            claim("the name the container has for it",
                    ctx.getBeanNamesForType(Kitchen.class)[0]);
        }
    }
