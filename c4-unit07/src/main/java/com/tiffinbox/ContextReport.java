...
public final class ContextReport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (AnnotationConfigApplicationContext ctx =
            ...
            header(stable, "c4-unit07");
            List<String> mine = appBeans(ctx.getBeanFactory());
            printBeans(ctx, mine, stable);

            System.out.println("claims");
            // The callbacks this run recorded, counted by the instrument that recorded them.
            // The context above has refreshed, so these are the STARTING callbacks only.
            int starting = Trace.count();
            if (starting == 0) {
                die("the instrument recorded 0 callbacks, so every number below is a report "
                        + "that did not run.");
            }
            claim("callbacks recorded while starting", String.valueOf(starting));
            org.springframework.beans.factory.config.BeanDefinition kbd =
                    ctx.getBeanFactory().getBeanDefinition("kitchen");
            claim("init method in the description",
                    String.valueOf(kbd.getInitMethodName()));
            claim("destroy method in the description",
                    String.valueOf(kbd.getDestroyMethodName()));
            claim("callbacks named by Kitchen.java", String.valueOf(namedInTheClass()));
            claim("kitchen.customers()", String.valueOf(ctx.getBean(Kitchen.class).customers()));
        }
    }

    /**
     * DERIVED off the class, not typed: how many of the callbacks are declared BY Kitchen.java
     * — the interfaces it implements plus its annotated methods. The rest are named elsewhere.
     */
    static int namedInTheClass() {
        int n = 0;
        for (Class<?> i : Kitchen.class.getInterfaces()) {
            if (i.getName().startsWith("org.springframework")) {
                n++;
            }
        }
        for (java.lang.reflect.Method m : Kitchen.class.getDeclaredMethods()) {
