...
public final class ContextReport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (AnnotationConfigApplicationContext ctx =
                     new AnnotationConfigApplicationContext(Rail.Config.class)) {
            header(stable, "c4-unit03/exercise");
            List<String> mine = appBeans(ctx.getBeanFactory());
            printBeans(ctx, mine, stable);

            System.out.println("claims");
            int cooks = ctx.getBean(Rail.Sizer.class).cooksNeeded();
            claim("sizer.cooksNeeded()", String.valueOf(cooks));
            claim("customers in the repository", String.valueOf(
                    ctx.getBean(CustomerRepository.class).findAllQuietly().size()));
            if (cooks == 0) {
                System.out.println("  ^ a context that started, and a kitchen sized for nobody.");
            }
        }
    }
