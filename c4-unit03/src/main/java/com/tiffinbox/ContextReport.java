...
public final class ContextReport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (AnnotationConfigApplicationContext ctx =
                     new AnnotationConfigApplicationContext(ThreeInjectionPoints.Config.class)) {
            header(stable, "c4-unit03");
            List<String> mine = appBeans(ctx.getBeanFactory());
            printBeans(ctx, mine, stable);

            System.out.println("claims");
            ThreeInjectionPoints.ByField f = ctx.getBean(ThreeInjectionPoints.ByField.class);
            claim("constructor: repo after refresh", ctx.getBean(
                    ThreeInjectionPoints.ByConstructor.class).state());
            claim("setter:      repo after refresh", ctx.getBean(
                    ThreeInjectionPoints.BySetter.class).state());
            claim("field:       repo after refresh", f.state());
            claim("field:       repo IN constructor", f.seenFromConstructor
                    + "   <- the one the compiler cannot warn you about");
            // DERIVED, not typed - INCLUDING THE DENOMINATOR. It used to read `withFinal + " of 3"`
            // beside this very array, which is the one number in the line nothing measured.
            Class<?>[] styles = { ThreeInjectionPoints.ByConstructor.class,
                    ThreeInjectionPoints.BySetter.class, ThreeInjectionPoints.ByField.class };
            int withFinal = 0;
            for (Class<?> c : styles) {
                for (java.lang.reflect.Field fl : c.getDeclaredFields()) {
                    if (java.lang.reflect.Modifier.isFinal(fl.getModifiers())
                            && fl.getType() == CustomerRepository.class) {
                        withFinal++;
                    }
                }
            }
            claim("styles whose injected field is final", withFinal + " of " + styles.length);
        }
    }
