...
public final class ContextReport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (AnnotationConfigApplicationContext ctx =
                     new AnnotationConfigApplicationContext(Cycle.Cured.class)) {
            header(stable, "c4-unit10");
            List<String> mine = appBeans(ctx.getBeanFactory());
            printBeans(ctx, mine, stable);

            System.out.println("claims");
            // The cure, stated as a property of the GRAPH rather than of the startup.
            claim("beans that point back at something pointing at them",
                    String.valueOf(cycleCount(ctx, mine)));
            claim("billing and delivery hold the SAME tariff", String.valueOf(
                    ctx.getBean(Cycle.Billing.class) != null
                            && ctx.getBean("tariff") == ctx.getBean(Cycle.Tariff.class)));
            claim("billing.billFor(\"Ravi\")",
                    String.valueOf(ctx.getBean(Cycle.Billing.class).billFor("Ravi")));
            claim("delivery.chargeFor(\"Ravi\")",
                    String.valueOf(ctx.getBean(Cycle.Delivery.class).chargeFor("Ravi")));
            claim("what tariff depends on", java.util.Arrays.toString(
                    ctx.getBeanFactory().getDependenciesForBean("tariff")));
        }
    }

    /**
     * DERIVED off the container's own dependency bookkeeping, over the SAME filtered list the
     * report printed: a bean is in a cycle when something it depends on depends on it back.
     */
    static int cycleCount(AnnotationConfigApplicationContext ctx, List<String> mine) {
        int n = 0;
        for (String a : mine) {
            for (String b : ctx.getBeanFactory().getDependenciesForBean(a)) {
                if (java.util.Arrays.asList(ctx.getBeanFactory().getDependenciesForBean(b))
                        .contains(a)) {
                    n++;
                    break;
                }
