...
- `Orders.java` — `record Order(String customer, MealType type, int quantity)` with a `total()` method + `enum MealType { VEG(120), NON_VEG(150), JAIN(130) }`; prints `Order[customer=Ravi, type=VEG, quantity=2]` / `Ravi pays 240` / `true` / `VEG -> 120` / `NON_VEG -> 150` / `JAIN -> 130`. Run: `java Orders.java`
