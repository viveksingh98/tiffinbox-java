...
public final class ContextReport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (AnnotationConfigApplicationContext ctx =
            ...
            header(stable, "c4-unit06/exercise");
            ...
            claim("beans of type OrderQueue", String.valueOf(
                    ctx.getBeanNamesForType(OrderQueue.class).length));
            claim("  their names, sorted", String.join(", ", mon.names()));
            claim("List<OrderQueue> injected size", String.valueOf(mon.railCount()));
            claim("kitchenRail == deliveryRail", String.valueOf(
                    ctx.getBean("kitchenRail") == ctx.getBean("deliveryRail")));
            claim("candidates declared @Primary", String.valueOf(primaryCount(ctx)));
        }
    }

    /** DERIVED off the definitions: how many candidates claim to be the default. */
    static int primaryCount(AnnotationConfigApplicationContext ctx) {
        int n = 0;
        for (String name : ctx.getBeanNamesForType(OrderQueue.class)) {
            if (ctx.getBeanFactory().getBeanDefinition(name).isPrimary()) {
                n++;
            }
        }
        return n;
